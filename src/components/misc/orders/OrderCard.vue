<template>
  ...
  >
        ...
        <v-row :class="idx == 0 && showAllProducts ? 'mt-1' : null">
          ...
          <v-col cols="8" :md="isInDetail ? 11 : 10" class="text-dark">
            ...
            <div class="d-md-none d-block">
              ...
              >
                <span class="col-9">{{
                  product.productVariantValueValue
                }}</span>
                <span class="float-right col-3 text-end"
                  >x{{ product.orderVariantQty }}</span
                >
    ...
    <v-card-actions
      v-if="
        ((isMobile && order.orderDetail?.length > mobileMaxShowList) ||
          (!isMobile && order.orderDetail?.length > desktopMaxShowList)) &&
        !showAllProducts
      "
    ...
    <div class="text-dark float-right pe-md-4">
      <div class="d-flex flex-row align-items-center gap-2 mb-2">
        ...
        <p class="fs-md-3 fs-6 mb-0 fw-bold">
          RM {{ parseFloat(order?.orderTotalAmount).toFixed(2) }}
        </p>
      </div>
    </div>
  </v-card>
...
export default {
  ...
  methods: {
    getVariantPrice(product) {
      return parseFloat(product.orderVariantPrice).toFixed(2);
    },
  },
  watch: {
    isMobile() {
